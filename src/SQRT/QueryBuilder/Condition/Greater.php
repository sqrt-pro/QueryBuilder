...
class Greater extends Condition
{
  ...
  {
    return Query::QuoteKey($this->column) . ($this->or_equal ? '>=' : '>') . Query::Placeholder($this->column . '_gt', $prefix);
  }

  public function getBindedValues($prefix = null)
  {
    return array(Query::Placeholder($this->column . '_gt', $prefix) => $this->values);
  }
